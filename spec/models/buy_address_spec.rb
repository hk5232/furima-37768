  ...
  describe '商品購入' do
    ...
    context '商品購入できない場合' do
      it '郵便番号が空では保存できない' do
        @buy.postal_code = ''
        @buy.valid?
        expect(@buy.errors.full_messages).to include('郵便番号は必須項目です')
      end

      it '郵便番号がハイフンなしでは保存できない' do
        @buy.postal_code = '1111111'
        @buy.valid?
        expect(@buy.errors.full_messages).to include('郵便番号は000-0000の形でのみ登録可能です')
      end
      ...
      it 'address_city' do
        @buy.address_city = ''
        @buy.valid?
        expect(@buy.errors.full_messages).to include('市区町村は必須項目です')
      end

      it 'address_streetが空では保存できない' do
        @buy.address_street = ''
        @buy.valid?
        expect(@buy.errors.full_messages).to include('番地は必須項目です')
      end

      it 'source_idが---では保存できない' do
        @buy.source_id = 1
        @buy.valid?
        expect(@buy.errors.full_messages).to include('都道府県は必須項目です')
      end

      it 'phone_numberが空では保存できない' do
        @buy.phone_number = ''
        @buy.valid?
        expect(@buy.errors.full_messages).to include('電話番号は必須項目です')
      end

      it 'phone_numberが9桁以下では保存できない' do
        @buy.phone_number = '1111'
        @buy.valid?
        expect(@buy.errors.full_messages).to include('電話番号は10から11桁の半角数字でのみ登録可能です')
      end
      ...
      it 'tokenが空では購入できないこと' do
        @buy.token = nil
        @buy.valid?
        expect(@buy.errors.full_messages).to include('クレジットカードは全て必須項目です')
      end
    end
  end
end
